...
def test_get_response_instruct(openai_instruct_model_provider: OpenAIInstructModelProvider, monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda model, prompt, max_tokens: {
...
def test_add_user_message_instruct(openai_instruct_model_provider: OpenAIInstructModelProvider) -> None:
    msg = MagicMock(spec=Message)
    msg.content = "User message"
    msg.author.display_name = "Username"
    msg.channel.id = 1
...
def test_history_append_bot_instruct(openai_instruct_model_provider: OpenAIInstructModelProvider) -> None:
    ...
    async def run_test() -> None:
        message = "Ai response"
        await openai_instruct_model_provider._history_append_bot(message, channel_id)
...
def test_check_history_len_instruct(openai_instruct_model_provider: OpenAIInstructModelProvider, monkeypatch: MonkeyPatch) -> None:
    ...
    async def run_test() -> None:
        ...
        await openai_instruct_model_provider._history_append_bot(message, channel_id)
        await openai_instruct_model_provider._check_history_len(channel_id)
        assert len(openai_instruct_model_provider.history[channel_id]) == 1
